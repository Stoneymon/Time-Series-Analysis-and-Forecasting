...
test_x2 <- test[,!"grid_next_hour"]
test_y2 <- test[,"grid_next_hour"]


fit.ranger2 <- ranger(grid_next_hour ~ ., data = training, importance = "permutation")
plot_ly(training, x = training, y = ~grid_next_hour, name = "actual", type = 'scatter', mode = 'lines+markers') %>%
...
plot_ly(test, x = test$Time, y = ~grid_next_hour, name = "actual", type = 'scatter', mode = 'lines+markers') %>%
